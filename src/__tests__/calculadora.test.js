...
describe('calcularDescuento', () => {
  ...
  test('Lanza error si el precio es negativo', () => {
    expect(() => calcularDescuento(-50, 10)).toThrow('El precio no puede ser negativo');
  });

  test('Lanza error si el porcentaje es mayor a 100', () => {
    expect(() => calcularDescuento(100, 150)).toThrow('El porcentaje debe estar entre 0 y 100');
  });
});
...
describe('calcularTotal', () => {
  ...
  test('Lanza error si no se pasa un arreglo', () => {
    expect(() => calcularTotal('no es arreglo')).toThrow('Se esperaba un arreglo');
  });
});
...
// GRUPO 5: Pruebas para aplicarIVA
describe('aplicarIVA', () => {
  test('Aplica correctamente el 15% de IVA a un precio base de 100', () => {
    expect(aplicarIVA(100)).toBe(115);
  });

  test('Retorna 0 si el precio base es 0', () => {
    expect(aplicarIVA(0)).toBe(0);
  });

  test('Lanza un error controlado si el precio ingresado es negativo', () => {
    expect(() => aplicarIVA(-50)).toThrow('El precio no puede ser negativo');
  });
});